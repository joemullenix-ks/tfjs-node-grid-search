...
class Axis {
	constructor(typeEnum, boundBegin, boundEnd, progression) {
		...
		if (BOUNDS_DELTA === 0) {
			if (progression) {
				console.warn('bounds delta is zero; progression has no effect');
			}

throw new Error('untested 234235');
		}
		else {
	...
	CalculatePosition() {
		return this._boundBegin + this._progression.GetValue();
	}

	CheckComplete() {
		return this.CalculatePosition() > this._boundEnd;
	}
