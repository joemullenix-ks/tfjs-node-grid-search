...
class SessionData {
	constructor(tensorFlow, proofPercentage, rawInputs, rawTargets, totalInputNeurons, totalOutputNeurons) {
		...
		// move a portion of the cases into a 'proof' set, to be used after training to measure generalization
		this._proofInputsTensor = tensorFlow.tidy(() => { return tensorFlow.tensor(PROOF_INPUTS); });
		this._proofTargetsTensor = tensorFlow.tidy(() => { return tensorFlow.tensor(PROOF_TARGETS); });

		// move a portion of the cases into a 'proof' set, to be used after training to measure generalization
		this._trainingInputsTensor = tensorFlow.tidy(() => { return tensorFlow.tensor(rawInputs); });
		this._trainingTargetsTensor = tensorFlow.tidy(() => { return tensorFlow.tensor(rawTargets); });
	}
