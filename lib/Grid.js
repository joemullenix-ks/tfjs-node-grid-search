...
//TODO: Expose this one.
	...
	ProcessOptions(options) {
		if (options === null || options === undefined) {
			...
			options =	{
							// These options are left out by default:
							// writeResultsToPath: '<PATH>'
						};

			// these defaults pass through validation, to double-check our work
		}

		const ERROR_PREFIX = 'Grid option ';

		if (options.writeResultsToPath !== undefined) {
//TODO: Come up with a RegEx or tool to validate the file path. For now we'll take any string, and leave it to
//		the (eventual) FileIO call to catch and report bad paths.
			if (typeof options.writeResultsToPath !== 'string') {
				throw new Error(ERROR_PREFIX + '"writeResultsToPath" must be a valid directory.');
			}

			console.log('Results will be written to '
						+ (options.writeResultsToPath === ''
...
*/
		...
		if (typeof this._options.writeResultsToPath === 'string') {
...
//		We might prefer to offer a user-provided filename prefix, in the _options block.
//		...pending beta feedback.
			await FileIO.WriteResultsFile(	'Results_' + Date.now() + '.csv',
											this._options.writeResultsToPath,
	...
	TestModel(model, modelParams, duration) {
		...
		const PREDICTIONS_TENSOR = model.predict(	this._sessionData.GetProofInputsTensor(),
													{
														batchSize: modelParams.GetParam('batchSize'), //HARD-CODER; TODO: Add this as a managed Axis type, or define it ... somewhere.
//NOTE: 'verbose' is not implemented as of TF 2.7.0. The documentation is wrong, but it's noted in the lib (see model.ts).
	...
	async TrainModel(model, modelParams) {
		...
		const TOTAL_EPOCHS = modelParams.GetParam('epochs'); //HARD-CODER; TODO: Add this as a managed Axis type, or define it ... somewhere.
...
//NOTE: Validation is only done if we provide this "validationSplit" arg. It's necessary to track overfit and stuck.
							validationSplit: modelParams.GetParam('validationSplit'), //HARD-CODER; TODO: Add this as a managed Axis type, or define it ... somewhere.
							callbacks:
							{
