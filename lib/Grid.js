...
//TODO: Expose this one.
	...
	async Run() {
		for (let iterations = 0; !this._axisSetTraverser.GetTraversed(); ++iterations) {
			...
			this.TestModel(MODEL, MODEL_PARAMS, Date.now() - this._timeStartIteration);
...
//NOTE: 'verbose' is ignored as of TF 2.7.0. The documentation is wrong, but it's noted in the lib (see model.ts).
														verbose: false
													});
...
//TODO: We'll have stock logic here that 'scores' the predictions. This may be a fallback, in the event the user
//		does not send a callback, or it may be standard procedure. That design is TBD.
//		It will need to differentiate between classification and regression, and that's a bigger kettle of fish.
//		I suppose we can require the user to be explicit (e.g. 'networkType' constuctor param).
...
// 								onYield: (epoch, batch, logs) => { console.log('onYield', epoch, batch, logs); }
								...
								onEpochEnd: (epoch, logs) => {
									...
									this._callbackReportEpoch(DURATION_EPOCH, epoch, logs);
return;

//WORKING: These may be lifted into the callback, but they're also needed here for tracking overfit and stuck situations.
//		   Going to give this a bit of thought, brb...
