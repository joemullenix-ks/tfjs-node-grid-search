...
class ModelStatics {
	constructor(tensorFlow, axisSet, params) {
		console.assert(typeof tensorFlow === 'object'); //TODO: This increasingly feels stupid. Why not just require it?
...
const ACTIVATION_DEFAULT = 'relu'; //'elu'|'hardsigmoid'|'linear'|'relu'|'relu6'| 'selu'|'sigmoid'|'softmax'|'softplus'|'softsign'|'tanh'
		...
		console.assert(Math.floor(this._staticParams.totalCases / this._staticParams.batchSize)
						=== this._staticParams.totalCases / this._staticParams.batchSize); // even mult; doesn't really matter
...
ModelStatics.GENERATE_INITIALIZER_BIAS = () => {		// DEFAULT: zeroes
	return tf.initializers.constant({value: 0.1});
};

ModelStatics.GENERATE_INITIALIZER_KERNEL = () => {		// DEFAULT: glorotNormal
	return tf.initializers.heNormal({seed: Math.random()});
};

ModelStatics.GENERATE_ACTIVATOR_TEST = () => {
