...
//      unacceptable solution, long term.
        ...
        this._proofInputsTensor = TENSOR_FLOW.tidy(() => {
                return TENSOR_FLOW.tensor(PROOF_INPUTS as TFArrayStack);
            });

        // convert the training data to tensors, for the model-fit step
        this._trainingInputsTensor = TENSOR_FLOW.tidy(() => {
                return TENSOR_FLOW.tensor(rawInputs as TFArrayStack);
            });
 ...
 * @memberof SessionData
 */
function FindMean(data: Array<number>): number {
 ...
 * @memberof SessionData
 */
function FindStandardDeviation(data: Array<number>, mean: number): number {
...
//			  ...after you check TF's own utils, or course!
    ...
    for (let i = 0; i < TOTAL_FEATURES; ++i) {
        ...
        const FEATURE_STDEV = FindStandardDeviation(FEATURE_VALUE_TABLE[i],
                                                    FEATURE_MEAN);
