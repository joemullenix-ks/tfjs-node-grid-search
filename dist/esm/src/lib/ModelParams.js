...
class ModelParams {
    ...
    constructor(_dynamicParams, _staticParams) {
        ...
        for (const k in this._dynamicParams) {
            if (this._mergedParams[k] !== undefined) {
                throw new Error('Merging model params with a dynamic-static '
                    + 'collision (these are mutually exclusive); '
                    + 'key: ' + k);
            }
    ...
    GetBooleanParam(key) {
        ...
        if (typeof this._mergedParams[key] !== 'boolean') {
            throw new Error('param ' + key + ' is not Boolean: '
                + this._mergedParams[key]);
        }
    ...
    GetNumericParam(key) {
        ...
        if (typeof this._mergedParams[key] !== 'number') {
            throw new Error('param ' + key + ' is not a number: '
                + this._mergedParams[key]);
        }
    ...
    GetTextParam(key) {
        ...
        if (typeof this._mergedParams[key] !== 'string') {
            throw new Error('param ' + key + ' is not a string: '
                + this._mergedParams[key]);
        }
    ...
    WriteCSVLineValues() {
        ...
        for (const k in this._mergedParams) {
            ...
            //PERF: This is potentially overkill (we validate these early on in their
            //      lifecycle).
            //		I'm keeping it because that initial validation is not done w/ CSV writes
            //      in mind. It's much more focused on proper uint/bool/string.
            //		Redundancy is good, but if our file writes become perceptibly slower,
            //      this can go.
