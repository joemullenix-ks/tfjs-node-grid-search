...
test('instantiation w valid args; calculate score', () => {
    const modelTestStats = new ModelTestStats(VALID_DELTA_CORRECT, VALID_DELTA_INCORRECT, VALID_TOTAL_CORRECT, VALID_TOTAL_CASES);
...
test('instantiation throws on float correct', () => {
    expect(() => {
        const modelTestStats = new ModelTestStats(VALID_DELTA_CORRECT, VALID_DELTA_INCORRECT, 1.5, VALID_TOTAL_CASES);
    }).toThrow();
});
test('instantiation throws on too many correct', () => {
    expect(() => {
        const modelTestStats = new ModelTestStats(VALID_DELTA_CORRECT, VALID_DELTA_INCORRECT, 1 + VALID_TOTAL_CASES, VALID_TOTAL_CASES);
    }).toThrow();
});
