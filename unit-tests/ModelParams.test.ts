...
import { ModelParams } from '../src/main';
import { StringKeyedSimpleObject } from '../src/main';


test('instantiation', () => {
  const emptyModelParams = new ModelParams({}, {});

  expect(emptyModelParams).toBeInstanceOf(ModelParams);

  const leftModelParams = new ModelParams({testKeyL: 0}, {});

  expect(leftModelParams).toBeInstanceOf(ModelParams);

  const rightModelParams = new ModelParams({}, {testKeyR: 0});

  expect(rightModelParams).toBeInstanceOf(ModelParams);
});

test('instantiation throws on collision', () => {
  expect(() => {
    const modelParams = new ModelParams({testKey: 0}, {testKey: 0});

    expect(modelParams).toBeInstanceOf(ModelParams);
  }).toThrow();
});
...
test('pulls boolean params; throws on invalid', () => {
  ...
  const OBJECT_LEFT: StringKeyedSimpleObject = {};
  const OBJECT_RIGHT: StringKeyedSimpleObject = {};
  ...
  const modelParams = new ModelParams(OBJECT_LEFT, OBJECT_RIGHT);
