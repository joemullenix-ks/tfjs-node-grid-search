...
import { DataSetFetcher } from '../src/main';
...
describe('instantiation', () => {
	test('conjure the proper object', () => {
		expect(new EpochStats(1)).toBeInstanceOf(EpochStats);
	});
	...
	test('float trailDepth throws', () => {
		expect(() => {
			const epochStats = new EpochStats(1.5);
		}).toThrow();
	});
});

describe('update call with bad inputs', () => {
	const epochStats = new EpochStats(2);
